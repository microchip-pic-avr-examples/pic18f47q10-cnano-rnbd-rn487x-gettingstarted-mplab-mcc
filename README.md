...
# PIC18F47Q10 RNBD451 and RN487x User Guide
...
-   [Microchip PIC18F-Q\_DFP Series Device Support \(1.21.411\)](https://packs.download.microchip.com/) or newer
...
-   Pin mapping table between the MCU and the RNBD451 Click:
    ...
    |3|BT\_RST|RC7|RST|
-   Connect the development board of your choice and then connect the RNBD451 to the proper slot. The image below shows how to connect the RNBD451 Click to **Slot 2**.
...
2.  **Note:** The Validation Hardware used with Microchip Bluetooth Data Mobile App is shown in the process below. PIC18F47Q10 8-bit devices are used for reference.
3.  Open MCC by clicking Tools→Embedded→MPLAB® Code Configurator or click the MCC button.<br /> ![](images/GUID-359085BA-D8D7-47F8-AF8D-750837A36071-low.png)

    or

    ![](images/GUID-5ADA889B-5F93-48F2-B5EF-4EE824D9DEA5-low.png)

4.  If MCC is not available, install it. Navigate to Tools→Plugins. Under **Available Plugins** select "MPLAB® Code Configurator", ensure the checkbox is selected and press **Install**. Once the installation is completed, MCC will be available.![](images/GUID-FF1D230A-1359-45DE-9631-F96EA45455EE-low.png)
5.  On launching MCC, click the **Content Manager** button.
    ...
    -  Expand Drivers section and load the below version of UART Drivers
...
6.  Go to Device Resources→Libraries→rnbd-rn487x-host-driver→RNBD-RN487x.
    ...
    - Add the RNBD-RN487x from Device Resources to Project Resources as below:<br />   ![](images/GUID-75D4253D-49BC-487C-BFE3-4BF1438646EC-low.png)

    <br />

    - Once the RNBD-RN487x driver was loaded to the Project Resources panel, choose EUSART1 as "UART Hardware Dependency Selector", in which the GUI will be expanded for another configuration setting tab
    <br />

    ![](images/rnbd_inital_uart_driver.png)
    <br />

    - After the initial setup, populate the default RNBD-RN487x Configuration populate as below:<br />
...
8.  Generate example applications:
...
9.  Clock settings:
    -   Go to Project Resources→System→Clock Control
    ...
    -   Set "HF Internal Clock" to 32_MHz and "Clock Divider" to 1<br />
...
10. UART settings:
    -   **PIC**
        -   **Note:** The below UART selections are applicable for both the Transparent UART Application and Basic Data Exchange (EUSART1 and EUSART2).
...
This section explains the RNBD451 MPLAB X IDE project setup, code generation, and shows how to run the the Basic Data Exchange and Transparent UART applications with the PIC18F47Q10 microcontroller.

## RNBD Basic Data Exchange Pin Settings<a name="rnbd-basic-data-exchange-pin-settings"></a>

<br />

-   PIC18F47Q10 MCC Configuration - Basic Data Exchange Example<br />
...
1.   The Basic Data Exchange application uses only one instance of EUSART1 for data transfer.
2.   **Important:** For EUSART Rx1 and Tx1, deselect the checkboxes in the Analog, Slew Rate and Input Level Control columns, as shown below.
...
Configuration for PIC Basic Data Exchange was completed. Click [here](#rnbd-code-generation-&-compilation) for rnbd code generation and compilation section.
...
The PIC18F47Q10 MCU has only two instances of EUSART, so the second one was automatically loaded for the Communication Device Class (CDC) UART. If the device has multiple instances, they will be listed to facilitate the selection of the respective EUSARTs.

![](images/GUID-442D1FB2-1B4A-4D4E-9F6B-5B457F25EC50-low.png)

**Notes:**

1.   The Transparent UART application uses two instances of EUSART for data transfer, EUSART1 and EUSART2.
2.   Check that the Interrupt Driven was enabled for both EUSART1 and EUSART2.
3.   **Important:** For EUSART Rx1 and Tx1, deselect the checkboxes in the Analog, Slew Rate and Input Level Control columns, as shown below.
...
3.  Go to Pins, under Project Resources:
    ...
    Deselect the checkboxes in the Analog, Start High, Weak Pullup, Slew Rate and Input Level Control columns as shown in the below picture
...
3.  Call the Example_Initialized(); function in main() after SYSTEM_Initialize().![](images/GUID-F690E78C-B6C8-4EC8-BD6D-4555B1ABAEB6-low.png)

4.  Connect the development board of your choice and then connect the RN487x to the proper slot. The image below shows how to connect the RNBD Click to Slot 2.
...
5.  Click Clean and Build to the generated project:
...
The \#define DEMO\_PERIODIC\_TRANSMIT\_COUNT \(10000\) and \#define DEMO\_PERIODIC\_CHARACTER \(‘1’\)  statements used in the demonstration can be located at the top of the rnbd_example.c file.
...
5.  Once connected, the Microchip Bluetooth app identifies all the services and characteristics supported by the RNBD451 device, as shown in the following figure.

    ![](images/GUID-AC320004-8543-404F-806A-4C5E70E09E3B-low.png)

6.  Click on the Microchip Data Service option. Select the Microchip Data Characteristic (Write Notify indication) to receive the data in the mobile application.

    ![](images/GUID-65CA4B07-2C54-4799-A9AE-826FD83B676D-low.png)

7.  Select Listen for notifications on the application.
      -   Select "Listen for notifications" in the application. It may be required to enable app notifications on the phone. Data will be sent periodically to the device and will be visible beneath the Notify/Listen setting.

        -   Before turning on the Notify/Indicate setting:

            ![](images/GUID-C5223277-D33D-4366-8CA4-124D340F0554-low.png)

        -  After turning on the Notify/Indicate setting, the mobile application can read "1", data which was sent from the RN487x module.
...
3.  Select "PIC32CXBZ" to scan for available devices to connect with. The application automatically scans the area for Bluetooth devices within range. By default, the device should appear as "RNBD451_xxxx".
...
This section explains the RN487x MPLAB X IDE project setup, code generation, and shows how to run the the Basic Data Exchange and Transparent UART applications with the PIC18F47Q10 microcontroller.
...
1.   Basic Data Exchange application uses only one instance of EUSART1 for data transfer.
2.   **Important:** For EUSART Rx1 and Tx1 deselect the checkboxes in the Analog, Slew Rate and Input Level Control columns, as shown below.
...
2.  Include the header #include "mcc_generated_files/examples/rnbd_example.h" in main.c.
3.  Call the Example_Initialized(); function in main() after SYSTEM_Initialize().
...
5.  Click Clean and Build for the generated project.
...
5.  Once connected Microchip Bluetooth app identifies all the services and characteristics supported by the RN487x device as shown in the following figure.
...
2.  Launch the phone application
    ...
    Open the main menu and go to the **BLE UART** sub apps option.
...
3.  Select "BM70" to scan for available devices to connect with.
    ...
    The application automatically scans the area for Bluetooth devices within range. By default, the device should appear as "RN487x_xxxx".
